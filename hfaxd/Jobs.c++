...
static const struct {
    ...
    { T_USE_SSLFAX,	A_RUSR|A_WUSR|A_RADM|A_WADM|A_ROTH },
...
HylaFAXServer::replyJobParamValue(Job& job, int code, Token t)
{
	...
	return;
    case T_USE_SSLFAX:
	replyBoolean(code, job.usesslfax);
	return;
...
HylaFAXServer::jstatCmd(const Job& job)
{
    ...
    if (checkAccess(job, T_USE_SSLFAX, A_READ))
	jstatLine(T_USE_SSLFAX,"%s", boolString(job.usesslfax));
...
HylaFAXServer::newJob(fxStr& emsg)
{
    ...
    job->usesslfax = curJob->usesslfax;
...
{
    for (u_int i = 0; i < jobHosts.length(); i++) {
	if (strncmp(jobid, (const char*) jobHosts[i].id, jobHosts[i].id.length()) == 0) {
		...
		if (jobHostClient->callServer(emsg)) {
			...
			curJobHost = i;
			return (true);
		    } else emsg = fxStr::format("could not authenticate with job host: %s", (const char*) emsg);
		} else emsg = fxStr::format("could not connect to job host: %s", (const char*) emsg);
		return (false);
	    } else {
...
#define	MAXSPEC	20
		...
		*fp++ = c, c = *++cp;
		...
		break;
	    case '2':
		fprintf(fd, fspec, "N "[job.usesslfax]);
		break;
	    }
	} else
	    putc(*cp, fd);
