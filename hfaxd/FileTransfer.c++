...
typedef struct {
    ...
    uint32_t		link;			// offset to next directory
    uint32_t		xres[2];		// X resolution indirect value
    uint32_t		yres[2];		// Y resolution indirect value
} DirTemplate;
...
HylaFAXServer::retrievePageCmd(const char* name)
{
	...
	if (restart_point && !TIFFSetDirectory(tif, (tdir_t) restart_point)) {
		...
		file_size = sizeof (DirTemplate) +
		    sizeof (TIFFHEADER) + sizeof (uint16_t);
...
#define	TIFFdiroff(v) \
	...
	{ TIFFTAG_ROWSPERSTRIP,		TIFF_LONG,	1, (uint32_t) -1 },
...
#define	NTAGS	((TIFFdiroff(link)-TIFFdiroff(SubFileType)) / sizeof (HFClassicTIFFDirEntry))
    ...
    union { int32_t i; char c[4]; } u; u.i = 1;
    ...
    templ.StripByteCounts.tdir_offset = (uint32_t) file_size - sizeof (buf);
    float res;
    TIFFGetField(tif, TIFFTAG_XRESOLUTION, &res);
	templ.xres[0] = (uint32_t) res;
    TIFFGetField(tif, TIFFTAG_YRESOLUTION, &res);
	templ.yres[0] = (uint32_t) res;
