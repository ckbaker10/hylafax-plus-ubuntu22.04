...
enum Token {
    ...
    T_USE_ECM,	 T_ECMTYPE,	T_USE_TAGLINE,	T_USE_XVRES,	T_USE_COLOR,	T_USE_SSLFAX,	T_USRKEY,	T_VRES, T_CONNTIME, T_DURATION,
    /*
     * SNPP tokens.
     */
...
#define	S_SETGID	0x0800		// must explicitly force GID on files
    ...
    fxStr	ldapReqGroup;		// LDAP group users need to be members of for
					// fax server access
    fxStr	ldapBindQuery;		// query format used in LDAP bind
    fxStr	ldapSearchQuery;	// query format used in LDAP search
    fxStr	ldapSearchDN;		// DN used in LDAP search
    fxStr	ldapGroupAttribute;	// LDAP group attribute label
