...
SendFaxJob::setupConfig()
{
    ...
    useSSLFax = true;			// default is to permit SSL Fax
...
SendFaxJob::setConfigItem(const char* tag, const char* value)
{
    ...
    else if (streq(tag, "usesslfax"))
	setUseSSLFax(FaxConfig::getBoolean(value));
...
void SendFaxJob::setUseSSLFax(bool b)			{ useSSLFax = b; }
...
SendFaxJob::createJob(SendFaxClient& client, fxStr& emsg)
{
    ...
    IFPARM("USESSLFAX", useSSLFax, true)
    if (serverdocover) {
