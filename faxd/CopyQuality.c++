...
static	void setupCompression(TIFF*, u_int, u_int, uint32_t);
...
FaxModem::recvEndPage(TIFF* tif, const Class2Params& params)
{
    ...
    TIFFSetField(tif, TIFFTAG_FAXRECVPARAMS,	(uint32_t) params.encode());
#endif
	...
	(uint32_t) server.setPageTransferTime());
#endif
}
...
FaxModem::recvSetupTIFF(TIFF* tif, long, int fillOrder, const fxStr& id)
{
    ...
    TIFFSetField(tif, TIFFTAG_IMAGEWIDTH,	(uint32_t) params.pageWidth());
	...
	TIFFSetField(tif, TIFFTAG_FILLORDER,		(uint16_t) fillOrder);
    }
 ...
 */
static void
setupCompression(TIFF* tif, u_int df, u_int jp, uint32_t opts)
{
...
FaxModem::flushRawData(TIFF* tif, tstrip_t strip, const u_char* buf, u_int cc, fxStr& emsg)
{
    ...
    if (cc && TIFFWriteRawStrip(tif, strip, (tdata_t)buf, cc) == -1) {
	serverTrace("RECV: %s: write error", TIFFFileName(tif));
        abortPageRecv();
        emsg = "Write error to TIFF file {E052}";
    }
...
FaxModem::recvRecordEmsg(TIFF* tif, fxStr& emsg)
{
    ...
    uint16_t cd = TIFFCurrentDirectory(tif);
    if (cd > 0) {
...
FaxModem::fixupJPEG(TIFF* tif, fxStr& emsg)
{
    ...
    for (uint32_t i = 0; i < (pagesize - 6); i++) {
