...
/*
 * Setup the CallerID or DID information or whatever
 */
bool
FaxModem::setOrigin(FaxRequest& req)
{
...
FaxModem::recvNSF( const NSF& aNsf )
{
    ...
    if( nsf.stationIdFound() )
        protoTrace("NSF %sremote station ID: \"%s\"",
                   ...
                   nsf.getStationId());
    if (nsf.getFeatures()) {
	protoTrace("NSF features discovered: %x", nsf.getFeatures()[0]);
    }
    return (nsf.getFeatures());
}
 ...
 * post-page message.  The string is assumed to have 5
 * characters per page of the form:
 *
 *   xxxxM
 *
 * where xxxx is a 4-digit hex encoding of the session
 * parameters required to send the page data and M is
...
FaxModem::traceFCF(const char* dir, u_int fcf)
{
	...
	case FCF_PWD:
	    ...
	    break;
	case FCF_TSA:
	    fcfname = "TSA (transmitting subscriber internet address)";
	    break;
	case FCF_CSA:
	    fcfname = "CSA (called subscriber internet address)";
	    break;
	case FCF_PSA:
	    fcfname = "PSA (polled subaddress)";
	    break;
	case FCF_CIA:
	    fcfname = "CIA (calling subscriber internet address)";
	    break;
	case FCF_ISP:
	    fcfname = "ISP (internet selective polling address)";
	    break;
	default:
...
int FaxModem::getSSLFaxFd()
    { return server.getSSLFaxFd(); }
bool FaxModem::getSSLFaxConnection()
    { return server.getSSLFaxConnection(); }
int FaxModem::getModemFd()
    { return server.getModemFd(); }
...
FaxModem::correctPhaseCData(u_char* buf, u_long& pBufSize,
                            u_int fillorder, const Class2Params& params, uint32_t& rows)
{
    u_char* endOfData;
...
FaxModem::convertPhaseCData(u_char* buf, u_long& totdata, u_int fillorder, 
			    const Class2Params& params, const Class2Params& newparams, uint32_t& rows)
{
