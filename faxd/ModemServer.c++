...
void
ModemServer::setSSLFaxFd(int fd)
{
    sslFaxFd = fd;
    if (!fd) sslFaxConnection = false;
}
...
ModemServer::setServerStatus(const char* fmt, ...)
{
    ...
    fprintf(statusFile, "\n");
    fflush(statusFile);
    if (ftruncate(fileno(statusFile), ftell(statusFile)) < 0) {
	// What can we do?
    }
...
ModemServer::openDevice(const char* dev)
{
    ...
    if (modemFd < 0) {
	if (seteuid(euid) < 0) {
	    traceServer("%s: seteuid (euid) failed (%m)", dev);
	}
...
ModemServer::beginSession(const fxStr& number, gid_t uid)
{
	...
	uid_t euid = geteuid();
	if (seteuid(0) < 0) {
	    logError("Failed to seteuid (root): %s", strerror(errno));
	}
	...
	if (seteuid(euid) < 0) {
	    logError("Failed to seteuid (euid): %s", strerror(errno));
	}
    }
...
CallType ModemServer::modemAnswerCall(AnswerType atype, fxStr& emsg, const char* dialnumber, bool doSecondAnswer)
...
ModemServer::getModemChar(long ms, bool isquery)
{
	...
	/*
	 * Historically we only ever were concerned with the read() on modemFd.  Although
	 * a select() with a set timeval could have been used instead of the ABOR signal
	 * timer, it was done with the timer and no select(). However, with SSL Fax we
	 * need to use a select(), though, because we're watching both modemFd and sslFaxFd
	 * which is best done with a select().  We could use a set timeval on the select()
	 * instead of the ABOR signal timer, but since the ABOR signal timer has been
	 * working for so long now, we'll preserve its use and not use the set timeval on
	 * the select() even though that would probably be the logical thing to do here.
	 *
	 * Historically, on a timer expiry rcvCC was set to -1 due to the ABOR signal
	 * interrupting read().
	 */
	if (ms) startTimeout(ms);
	do {
	    if (!isquery && sslFaxFd) {
		// We have to watch both modemFd and sslFaxFd
		fd_set rfds;
		FD_ZERO(&rfds);
		FD_SET(modemFd, &rfds);
		FD_SET(sslFaxFd, &rfds);
#if CONFIG_BADSELECTPROTO
		if (select(modemFd > sslFaxFd ? modemFd+1 : sslFaxFd+1, (int*) &rfds, NULL, NULL, NULL) < 1) {
#else
		if (select(modemFd > sslFaxFd ? modemFd+1 : sslFaxFd+1, &rfds, NULL, NULL, NULL) < 1) {
#endif
		    // Timeout occurred.
		    rcvCC = -1;
		} else {
		    if (FD_ISSET(modemFd, &rfds)) {
			// Looks like we're not doing SSL Fax (yet?).  Read from the modem.
			rcvCC = Sys::read(modemFd, (char*) rcvBuf, sizeof (rcvBuf));
		    } else {
			// Looks like we've got an SSL Fax connection.
			rcvCC = -1;
			sslFaxConnection = true;
		    }
		}
	    } else {
		rcvCC = Sys::read(modemFd, (char*) rcvBuf, sizeof (rcvBuf));
