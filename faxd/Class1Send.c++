...
Class1Modem::getPrologue(Class2Params& params, bool& hasDoc, fxStr& emsg, u_int& batched, bool docng)
{
	...
	framerecvd = recvFrame(frame, FCF_SNDR, conf.t1Timer, true, true, true, 0, false, true);	// T1 is used here, handle stutter
    } else if (docng) {
...
failed:
    ...
    if (!emsg.length()) emsg = "Unspecified failure to train with receiver";
